...
class Scan(multiprocessing.Process):
    ''' 程序扫描进程，实例化读写器实例，通过提供的接口获取读写器有关信息 '''
    ...
    def callbackLook(self, handle, context):
        ''' 盘点回调查找 '''
    ...
    def FrequencySave(self, cards, DeviceID):
        '''
        频率保存
        '''
        try:
        ...
        while True:
            ...
            for i in range(0, len(threads)):
                ...
                if ip in states.keys():
                    state = int(states[ip]['state'])
                else:
                    state = 1

                if threads[i].isAlive() == False:
                    threads[i] = threading.Thread(target=self.scan, args=(ip, ))
                    threads[i].start()
